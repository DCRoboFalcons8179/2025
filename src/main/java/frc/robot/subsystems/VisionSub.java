...
public class VisionSub extends SubsystemBase {
  private PhotonCamera frontCamera =
      new PhotonCamera(Constants.Vision.FrontCameraValues.cameraName);
  ...
  /**
   * Sets the unreadResults for the frontCamera, only ever use {@link
   * #frontCamera.getAllUnreadResults()} in this method
   */
   ...
   *
   * @return PhotonTrackedTarget - Will return null if no target is found, HANDLE that using {@code
   *     (target != null)}
   */
  ...
  public double getDistance(List<PhotonPipelineResult> cameraUnreadResults) {
    ...
    if (!cameraUnreadResults.isEmpty()
        && cameraUnreadResults.get(0).hasTargets()
        && target != null) {
      return PhotonUtils.calculateDistanceToTargetMeters(
